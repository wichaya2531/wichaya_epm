...
export const GET = async (req) => {
    ...
    const jobtemplate=await JobTemplate.find({});
    //console.log("jobtemplate",jobtemplate);
    //jobtemplate.forEach()
    var data="";
    var n=0;
    for (const job of jobtemplate) {
                if (n==0) {
                        console.log("job",job);
                }  

                var workgroup=await Workgroup.findById(job.WORKGROUP_ID );    
                ...
                try {
                    ...
                    //console.log("WORKGROUP_NAME:",workgroup.WORKGROUP_NAME);    
                    data+=job.JOB_TEMPLATE_NAME+"|"+workgroup.WORKGROUP_NAME+"|"+formattedDate+"\r\n";
                } catch (error) {
                    data+=job.JOB_TEMPLATE_NAME+"|Unknow|"+formattedDate+"\r\n";
                }
               
                n++;  
                //console.log(job.JOB_TEMPLATE_NAME+","+job.createdAt);
