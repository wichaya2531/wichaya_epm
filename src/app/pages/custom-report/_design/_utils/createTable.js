...
const createTable = async ({
    ...
    if(value.isConfirmed) {
        ...
        setCustomReportProfiles(profiles => profiles.map(profile => (
            profile.id === currentCustomReportProfileId ? {
                ...profile,
                customReportTables: [
                    ...profile.customReportTables.slice(0, before ? tableIndex : tableIndex + 1),
                    newTable,
                    ...profile.customReportTables.slice(before ? tableIndex : tableIndex + 1),
                ]
            } : profile
        )))
    }
